...
//
// This is a terrible hack: Because the original FTP task does not support proxies, I had to copy this source file and
// amend it. (Deriving from the original task and overriding individual methods doesn't work, because the code
// accesses many PRIVATE fields.)
//
// Also, some helper classes had to be copied, and the package renamed to "net2" to avoid a clash of classes with
// the same name but loaded by different class loaders:
//    java.lang.IllegalAccessError: class org.apache.tools.ant.taskdefs.optional.net.FTP2 cannot access its
//    superinterface org.apache.tools.ant.taskdefs.optional.net.FTPTaskConfig").
//

//
// Some extra documentation:
// http://www.codeproject.com/Articles/9547/How-Proxy-Server-serves-FTP-clients
// describes various authentication schemes that various FTP servers support:
//
// (1)
//   user [<proxy-user>]
//   pass [<proxy-password>]
//   user <user>@<server>        // No suffix ":<port>" allowed here!
//   pass <password>
//
// (2) http://www.rebex.net/ftp-ssl.net/features/proxy.aspx, "FtpSite"
//   user [<proxy-user>]
//   pass [<proxy-password>]
//   site <server>[:<port>]      // "SITE = Sends site specific commands to remote server"
//   user <user>
//   pass <password>
//
// (3)
...
//   user <user>@<server>
//
// (5) http://www.rebex.net/ftp-ssl.net/features/proxy.aspx, "FtpOpen"
//   user [<proxy-user>]
//   pass [<proxy-password>]
//   open <server>[:<port>]      // OPEN is not an official FTP command!
//   [user <user>]
//   [pass <password>]
//
// (6)
...
//
// (7) http://www.rebex.net/ftp-ssl.net/features/proxy.aspx, "FtpDoubleLogin"
//   user <proxy-user>@<server>
//   pass <proxy-password>
//   user <user>
//   pass <password>
//
// (8) http://www.rebex.net/ftp-ssl.net/features/proxy.aspx, "FtpUser"
//   user <user>@<proxy-user>@<server>[:<port>]
//   pass <password>@<proxy-password>
//
// The FTP proxy of "McAfee Web Gateway" appears to support schemes #1, #2 and #5.
// This task implements scheme #1.
//
// Another source of information on FTP proxy authentication:
//   http://www.rebex.net/ftp-ssl.net/features/proxy.aspx
//
 ...
 *
 */
package org.apache.tools.ant.taskdefs.optional.net2;
 ...
 * @since Ant 1.3
 */
@SuppressWarnings({ "rawtypes", "serial", "unchecked", "unqualified-field-access" })
public class FTP2 extends Task implements FTPTaskConfig {
    ...
    /**
     * If set, then the connection to the {@link #setServer(String)} is not established directly, but through the
     * designated FTP proxy server.
     *
     * @see InetAddress#getByName(String)
     */
    public void setProxyServer(String hostName) {
        this.proxyServer = "".equals(hostName) ? null : hostName;
    }

    /**
     * The port of the {@link #setProxyServer(String)} to use.
     *
     * @ant.defaultValue {@value #DEFAULT_FTP_PORT}
     */
    public void setProxyPort(int portNumber) {
    ...
    public void setProxyUserid(String userId) {
        this.proxyUserid = "".equals(userId) ? null : userId;
    }

    /**
     * @see #setProxyUserid(String)
     */
    public void setProxyPassword(String password) {
        this.proxyPassword = "".equals(password) ? null : password;
    }
    ...
    protected void sendFile(FTPClient ftp, String dir, String filename)
        ...
        try {
            ...
            if (newerOnly && isUpToDate(ftp, file, resolveFile(filename))) {
                if (verbose) log("No need to transfer " + filename + " (" + file.getAbsolutePath() + " is up-to-date)");
                return;
            }
